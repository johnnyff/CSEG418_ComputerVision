	...
	*****/
	...
	for (i = 0; i < Proj_q.cols; i++) {
		for (j = 0; j < Proj_p.cols; j++) {
			dist.at<float>(i, j) = cv::norm(Proj_q(Rect(i, 0, 1, Proj_q.rows)), Proj_p(Rect(j, 0, 1, Proj_p.rows)), NORM_L2);
		}
		///////////////////////////////////////////////////////////////////////
...
void CPCAfaceRecognitionView::OnFileOpen()
{
	...
	*****/
	...
	//m_cs.Format("ROIQ cols %d , rows %d ", Proj_p.cols, Proj_p(Rect(0, 0, 1, 50)).rows);
	//pDC.TextOut(0, 20, m_cs);
	...
	// This method will not run until this part is completed                                     //
	cv::sortIdx(dist, m, cv::SORT_EVERY_ROW);	
	////////////////////////////////////////////////////////////////////////////////////////////////
